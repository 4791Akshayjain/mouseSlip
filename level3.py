...
def run_level(screen, cursor_img, clock):
    ...
    # Load progress to determine hearts / difficulty
    data = load_save()
    ...
    x, y = start_px
    dragging = False
    drag_lockout = False   # to prevent dragging after hitting a wall to prevent multiple hits
    goal_radius = 10
    ...
    while True:
        ...
        try:   #exp remove in case of error
            x, y
        except NameError:
            x, y = start_px
        ...
        if buttons[0]:
            ...
            dragging = False
            drag_lockout = False   # clear lockout once button is released
        ...
        if not maze_rect.contains(char_rect):
            ...
            # For retry behavior we just return RETRY (main will reload save for next attempt)
            return "RETRY"
        ...
        for w in wall_rects:
            ...
            if w.colliderect(char_rect):
                if current_time - last_hit_time >= invincibility_time:
                    last_hit_time = current_time
                    if isinstance(data.get("hearts", "default"), int):
                        # decrement global hearts and persist
                        data["hearts"] = int(data["hearts"]) - 1
                        save_progress(data)
                        health = data["hearts"]
                        if data["hearts"] <= 0 and data.get("difficulty") == "hard":
                            # hard-fail: reset progression and hearts, then return special code
                            data["highest_completed_seq_index"] = -1
                            save_progress(data)
                            return "BACK_TO_MENU_HARD_FAIL"
                        # reposition to nearest safe cell
                        safe_x, safe_y = find_nearest_safe_cell(x, y)
                        x, y = safe_x, safe_y

                        # Move OS mouse to safe cell (best-effort)
                        try:
                    ...
                    # If no health left in non-hard global mode or local mode -> retry
                ...
                break  # stop checking more walls this frame
