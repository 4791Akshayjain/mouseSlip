...
def bfs_next_step(start, goal, maze, shield_blocks, offset_x, offset_y):
    ...
    while queue:
        ...
        for dx, dy in dirs:
            ...
            if maze[ny][nx] == WALL:
                continue
            # if shield blocks are present, skip cells whose rect collides a shield rect
            if shield_blocks:
...
def run_level(screen, cursor_img, clock):
    ...
    # desired spawn; we snap to nearest safe PATH cell if needed
    ...
    while True:
        ...
        # Instant death if outside
        if not maze_rect.contains(char_rect):
            # use hearts system if present
        ...
        # If shield is active and spider is inside shield block, it won't damage player
