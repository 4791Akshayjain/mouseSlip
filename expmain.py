# expmain.py
# Launcher for Mouse Slip — minimal menu + difficulty-on-new-game behavior
import pygame as pg
import sys
import os
import json
...
try:
    ...
    if os.path.exists(cursor_img_path):
        ...
        cursor_img = pg.transform.smoothscale(cursor_img, (24, 24))
    else:
        raise FileNotFoundError
except Exception:
    cursor_img = pg.Surface((24, 24), pg.SRCALPHA)
    pg.draw.circle(cursor_img, (255, 255, 255), (12, 12), 10)

# ---------------------- import levels ----------------------
# keep your LEVEL_ORDER and LEVELS mapping as in your example
import level1, level2, level3, level4, level5, level6, level7, level8, level9, level10, level11, level12

LEVEL_ORDER = [1, 2, 3, 5, 7, 4, 6, 8, 9, 10, 11, 12]

LEVELS = {
    1: level1, 2: level2, 3: level3, 4: level4, 5: level5,
    6: level6, 7: level7, 8: level8, 9: level9, 10: level10, 11: level11, 12: level12
}
...
def default_save():
    # include hearts/difficulty for compatibility with your level files
    return {
...
def save_progress(data):
    ...
    try:
        ...
        # fallback: write to tempdir (best-effort)
        try:
            import tempfile
...
COLOR_BG = (30, 30, 30)      # same as level background
COLOR_BTN = (200, 30, 30)    # red buttons
...
def process_events():
    ...
    for e in pg.event.get():
        if e.type == pg.QUIT:
            ...
            if e.button == 1:
                clicked = True
            elif e.button == 4:  # wheel up (older pygame)
                wheel += 1
            elif e.button == 5:  # wheel down (older pygame)
                wheel -= 1
        elif e.type == pg.MOUSEWHEEL:  # pygame 2
            wheel += e.y
    return quit_game, clicked, esc, wheel

# ---------------------- utility: is save "empty" (no player data) ----------------------
def save_is_empty(data):
    
   
    if data is None:
        return True
    # Normalize loaded data so we can compare reliably
    norm = {
...
# ---------------------- difficulty selection (only for new players) ----------------------
def difficulty_selection_modal():
    """
    A simple modal that forces the player to choose Normal or Hard.
    Returns "normal" or "hard" (also saves the choice).
    """
    ...
    while True:
        ...
        if quit_game:
            ...
            sys.exit()
        if esc:
            # don't allow escape — force a choice (you can change this)
            pass
        ...
        if mb and clicked:
            if hovered_easy:
                ...
                data["hearts"] = "default"
                # keep highest_completed_seq_index = -1 (new game)
                save_progress(data)
                return "normal"
            if hovered_hard:
                data = load_save()
                ...
                # starting a hard new game should reset progress (explicit)
                data["highest_completed_seq_index"] = -1
                save_progress(data)
                return "hard"
...
# ---------------------- run a single level by number ----------------------
def run_level_by_number(level_number):
    module = LEVELS[level_number]
    # calls: run_level(screen, cursor_img, clock)
    while True:
...
def menu_loop():
    while True:
        ...
        if clicked:
            if hovered_play:
                # If save is empty (new player), force difficulty selection modal before starting first run
                data = load_save()
...
def level_select_loop():
    ...
    while True:
        ...
        # update scrolling using wheel (positive -> scroll up)
        scroll_y -= wheel * 30
        ...
        if clicked:
            ...
            if hover_level_number is not None:
                ...
                data = load_save()
                elapsed = res
                record_completion_time(data, hover_level_number, elapsed)
