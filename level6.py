...
def create_teleporting_blocks(maze, num_blocks):
...
def valid_teleport_targets(maze, block, excluded_blocks):
    ...
    # require at least a 4-cell gap in either axis
    return [(tx, ty) for (tx, ty) in path_cells if abs(ox - tx) >= 3 and abs(oy - ty) >= 3]
