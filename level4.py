...
def create_pushable_blocks(maze, num_blocks=5):
...
def run_level(screen, cursor_img, clock):
    ...
    x, y = start_px
    dragging = False
    drag_lockout = False   # prevents immediate re-drag after a hit
    goal_radius = 10

    # Load heart image if available
    try:
    ...
    # Create pushable blocks (store as list of (bx,by) cell coordinates)
    blocks = create_pushable_blocks(_maze, num_blocks=6)
    ...
    # helper to convert pixel pos to cell coords (clamped)
    def pixel_to_cell(px, py):
        cx = (px - offset_x) // CELL_SIZE
        cy = (py - offset_y) // CELL_SIZE
    ...
    while True:
        ...
        if buttons[0]:
            ...
            dragging = False
            drag_lockout = False  # clear lockout when button released

        if dragging:
            # Determine pushing direction (cardinal)
            dx, dy = mx - x, my - y
            if abs(dx) > abs(dy):
            ...
            # Player cell (rounded)
            player_cell = pixel_to_cell(x, y)
            for i, (bx, by) in enumerate(blocks):
                if player_cell == (bx, by):
                    target_cell = (bx , by)
                    # bounds check
                    if 0 <= target_cell[0] < COLS and 0 <= target_cell[1] < ROWS:
                        if _maze[target_cell[1]][target_cell[0]] == PATH and target_cell not in blocks:
                            blocks[i] = target_cell

        # redraw char rect after movement
        char_rect = pg.Rect(x - HITBOX_SIZE // 2, y - HITBOX_SIZE // 2, HITBOX_SIZE, HITBOX_SIZE)
                    ...
                    if isinstance(data.get("hearts", "default"), int):
                        ...
                        if data["hearts"] <= 0:
                            if data.get("difficulty") == "hard":
                                # let launcher handle global reset
                        ...
                        try:
                            ...
                            pass
                break  # only handle one collision that frame

        # Draw pushable blocks
        for bx, by in blocks:
            rect = pg.Rect(offset_x + bx * CELL_SIZE, offset_y + by * CELL_SIZE, CELL_SIZE, CELL_SIZE)
            pg.draw.rect(screen, (205, 12, 14), rect)
        ...
        text_surface = font.render("Visuals can be deceptive", True, (255, 255, 255))
        screen.blit(text_surface, (screen_w // 2 - text_surface.get_width() // 2, 10))
